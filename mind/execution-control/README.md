...
It is the runtime control responsibility inside `Mind`. Its job is to advance the `Plan` generated by `Planning` into actual task execution.
...
See `tasks/TEMPLATE.md §Required Reads Map` for the formal list of files that must be read before running `Execution Control`.
