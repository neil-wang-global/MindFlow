...
- High
- Official Kubernetes project blog, authored by William Morgan (Buoyant/Linkerd)
...
## Fetch Completeness
- truncated
- WebFetch returned content truncated mid-sentence near the end of the article; the core technical content about the problem and approaches was preserved

## Original Content

gRPC Load Balancing on Kubernetes without Tears

By William Morgan (Buoyant) | Wednesday, November 07, 2018

Note: This article is more than one year old. Older articles may contain outdated content. Check that the information in the page has not become incorrect since its publication.
...
Now back to gRPC. Since we can't balance at the connection level, in order to do gRPC load balancing, we need to shift from connection balancing to request balancing. In other words, we need to open an HTTP/2 connection to each destination, and balance requests across these connections.
